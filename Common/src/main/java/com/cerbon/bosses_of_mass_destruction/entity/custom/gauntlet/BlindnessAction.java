        ...
        eventScheduler.addEvent(
                new TimedEvent(
                        () -> {
                            ...
                            if (!players.isEmpty()){
                                Dispatcher.sendToClientsLoadingPos(new BlindnessS2CPacket(entity.getId(), players.stream().flatMapToInt(player -> IntStream.of(player.getId())).toArray()), serverLevel, entity.position());
                                eventScheduler.addEvent(
                                        new TimedEvent(
