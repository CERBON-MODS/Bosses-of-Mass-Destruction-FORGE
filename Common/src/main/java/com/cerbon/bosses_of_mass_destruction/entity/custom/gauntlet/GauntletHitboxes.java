...
public class GauntletHitboxes implements IDamageHandler {
    ...
    public void setOpenHandHitbox(){
        if (!entity.level().isClientSide() && currentHitbox != hitboxes){
            Dispatcher.sendToClientsLoadingPos(new ChangeHitboxS2CPacket(entity.getId(), true), (ServerLevel) entity.level(), entity.position());
        }
    ...
    public void setClosedFistHitbox() {
        if (!entity.level().isClientSide() && currentHitbox != clampedHitboxes){
            Dispatcher.sendToClientsLoadingPos(new ChangeHitboxS2CPacket(entity.getId(), false), (ServerLevel) entity.level(), entity.position());
        }
