...
public class LichUtils {
    ...
    public static final Function<Float, Vector3d> blueColorFade = f -> MathUtils.lerpVec(f, BMDColors.COMET_BLUE, BMDColors.FADED_COMET_BLUE);
    ...
    public static void cappedHeal(IEntity iEntity, IEntityStats stats, List<Float> hpPercentRageModes, float healingStrength, Consumer<Float> heal) {
        if (iEntity.isAlive()) {
            ...
            float healAmt = MathHelper.clamp(targetHealthRatio * stats.getMaxHealth() - stats.getHealth() - 1, 0f, healingStrength);
